...
/////////////////////////////////////////////////////////////////////////////////////
void Alignment::Compress(const char infile[])
{
...
inline int Alignment::FilterForDisplay(int max_seqid, int coverage, int qid, float qsc, int N)
{
  ...
  v=0;
  n_display=0;
  if (kss_dssp>=0) display[kss_dssp]=0;
  if (ksa_dssp>=0) display[ksa_dssp]=0;
  if (kss_pred>=0) display[kss_pred]=0;
  if (kss_conf>=0) display[kss_conf]=0;
...
//      printf("  qsc=%6.2f     qid=%6.2f  \n",qsc_sum/nres[k],100.0*(1.0-(float)(diff)/nres[k]));
  ...
  if (seqid1>seqid2)
    {
      for (n=k=0; k<N_in; ++k) if (keep[k]>0) n++;
      return n;
    }
      ...
      for (kk=0; kk<N_in; ++kk)
        {
          ...
          if (keep[k]==2) {inkk[kk]=2; continue;} // accept all marked sequences (no n++, since this has been done already)
  ...
  for (i=1; i<=L; ++i) // Calculate wi[k] at position i as well as Neff[i]
    {
      ...
      q.tr[i][M2M]=log2n(q.tr[i][M2M]/sum);
      q.tr[i][M2I]=log2n(q.tr[i][M2I]/sum);
      q.tr[i][M2D]=log2n(q.tr[i][M2D]/sum);
  ...
  if (par.wg==1)
    {
      ...
      float scale=log2n((Nlim-q.Neff_HMM)/(Nlim-1.0)); // for calculating Neff for those seqs with inserts at specific pos
      for (i=1; i<=L; ++i)
        {
  ...
  if (1)
    {
      ...
      scale=log2n((Nlim-q.Neff_HMM)/(Nlim-1.0)); // for calculating Neff for those seqs with inserts at specific pos
    }
...
//            fprintf(stderr,"I  i=%3i  ncol=%3i  Neff_M=%5.2f  Nlim=%5.2f  w_I=%5.3f  Neff_I=%5.2f\n",i,ncol,q.Neff_HMM,Nlim,w_I,Neff[i]);
      ...
      q.tr[i][I2M]=log2n(q.tr[i][I2M]/sum);
      q.tr[i][I2I]=log2n(q.tr[i][I2I]/sum);
  ...
  if(1)
    {
      for (k=0; k<N_in; ++k) wi[k]=wg[k];
      Nlim=fmax(10.0,q.Neff_HMM+1.0);    // limiting Neff
      scale=log2n((Nlim-q.Neff_HMM)/(Nlim-1.0)); // for calculating Neff for those seqs with dels at specific pos
    }
...
//            fprintf(stderr,"D  i=%3i  ncol=%3i  Neff_M=%5.2f  Nlim=%5.2f  w_D=%5.3f  Neff_D=%5.2f\n",i,ncol,q.Neff_HMM,Nlim,w_D,Neff[i]);
      ...
      q.tr[i][D2M]=log2n(q.tr[i][D2M]/sum);
      q.tr[i][D2D]=log2n(q.tr[i][D2D]/sum);
