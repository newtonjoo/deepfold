...
void SetSubstitutionMatrix()
{
  ...
  for (a=0; a<20; ++a)
    for (b=0; b<20; ++b)   
      S[a][b] = log2n(R[a][b]/pb[a]); // S[a][b] = log2(P(a,b)/P(a)/P(b))
  ...
  if (v>=3)
    {
      ...
      for (a=0; a<20; ++a) entropy_pb-=pb[a]*log2n(pb[a]);
      for (a=0; a<20; ++a) 
	  for (b=0; b<20; ++b) 
	    {
	      entropy-=P[a][b]*log2n(R[a][b]);
