...
"""Wrapped optimizer for training DeepFold."""
...
class Optimizer:
  ...
  def __init__(self,
    ...
    self.warm_up_schedule = linear_warm_up(
        peak_lr=peak_lr,
        num_steps=(warm_up_steps+1))
    
    if self.config.decay.name == 'piecewise_constant':
      self.decay_schedule = opt.piecewise_constant(self.config.decay.boundaries, self.config.decay.decays)
    else:
