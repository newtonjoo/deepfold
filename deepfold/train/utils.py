...
"""Utilities for training Deepfold."""
...
from deepfold.common.residue_constants import restype_order_with_x
from deepfold.data.mmcif_parsing import MmcifObject
...
def load_labels(cif_path: str, pdb_id: str, chain_id: str = 'A') -> FeatureDict:
  ...
  if mmcif_obj is not None:
    ...
    logging.error(f'mmcif_obj is None: {cif_path}, {pdb_id}, {chain_id}')
    exit()
  return {
    ...
    'sequence':           sequence              # NR by newton
  }
...
def crop_and_pad(
  ...
  if num_res < crop_size and pad_for_shorter_seq:
    ...
    crop_start = np.clip(crop_start, 0, num_res - crop_size - 1)
  ...
  return raw_features, raw_labels, crop_start
