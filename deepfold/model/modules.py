...
"""Modules and code used in the core part of DeepFold.

The structure generation code is in 'folding.py'.
"""
from jax.experimental.host_callback import id_tap
def ascii_print(arg, transforms):
  print("".join(chr(i) for i in arg))
def val_print(arg, transforms):
  print(arg)
def jax_print(msg):
  if type(msg) == str:
    id_tap(ascii_print, jnp.array(list(msg.encode('ascii'))))
  else:
    id_tap(val_print, msg)
from deepfold.train.tensorboard import jax_log

import functools
from deepfold.common import residue_constants
from deepfold.model import all_atom
from deepfold.model import common_modules
from deepfold.model import folding
from deepfold.model import layer_stack
from deepfold.model import layer_norm
from deepfold.model import lddt
from deepfold.model import mapping
from deepfold.model import prng
from deepfold.model import quat_affine
from deepfold.model import utils
import haiku as hk
import jax
...
class AlphaFoldIteration(hk.Module):
  ...
  def __init__(self, config, global_config, name='deepfold_iteration'):
  ...
  def __call__(self,
    ...
    if not self.config.embeddings_and_evoformer.skip_module:
      ...
      if self.config.embeddings_and_evoformer.stop_gradient:
        representations = jax.tree_map(jax.lax.stop_gradient, representations)
    else:
      # retrive pre-build representations for efficiency
      representations = {}
      batch0 = slice_batch(0)
      representations['input_pair'] = batch0['teacher_input_pair']
      representations['single'] = batch0['teacher_single']
      representations['pair'] = batch0['teacher_pair']
      num_sequences = batch0['msa_feat'].shape[0]
      representations['msa'] = batch0['teacher_msa'][:num_sequences, :, :]
      representations['msa_first_row'] = batch0['teacher_msa'][0]
    ...
    for head_name, head_config in sorted(self.config.heads.items()):
      ...
      head_factory = {
          ...
          'predicted_sc_confidence': PredictedSCConfidenceHead,
    ...
    do_stop_gradient = {
        "masked_msa": self.config.heads.masked_msa.stop_gradient,
        "distogram": self.config.heads.distogram.stop_gradient,
        "structure_module": self.config.heads.structure_module.stop_gradient,
        "predicted_lddt": self.config.heads.predicted_lddt.stop_gradient,
        "predicted_sc_confidence": self.config.heads.predicted_sc_confidence.stop_gradient,
        "predicted_aligned_error": self.config.heads.predicted_aligned_error.stop_gradient,
        "experimentally_resolved": self.config.heads.experimentally_resolved.stop_gradient,
    }
    
    for name, (head_config, module) in sorted(heads.items()):
      # Skip PredictedLDDTHead and PredictedAlignedErrorHead until
      # StructureModule is executed.
      if name in ('predicted_lddt', 'predicted_aligned_error', 'predicted_sc_confidence', 'distillation'):
        continue
      else:
        # jhw - providing the predicted distogram for structure module
        if name == 'structure_module':
          batch['distogram'] = ret['distogram']
        
        ret[name] = module(representations, batch, is_training)
        if do_stop_gradient[name]:
          ret[name] = jax.tree_map(jax.lax.stop_gradient, ret[name])
    ...
    if self.config.heads.get('predicted_sc_confidence.weight', 0.0):
      # Add PredictedSCConfidenceHead after StructureModule executes.
      name = 'predicted_sc_confidence'
      # Feed all previous results to give access to structure_module result.
      head_config, module = heads[name]
      ret[name] = module(representations, batch, is_training)
      if do_stop_gradient[name]:
    ...
    #distillation loss by cy3
    if ('distillation' in self.config.heads
        and self.config.heads.get('distillation.weight', 0.0)):
      # Add DistillationHead after StructureModule executes.
      name = 'distillation'
      # Feed all previous results to give access to structure_module result.
      head_config, module = heads[name]
      ret[name] = representations
      if compute_loss:
        total_loss += loss(module, head_config, ret, name, filter_ret=False)
...
class AlphaFold(hk.Module):
  ...
  def __init__(self, config, name='deepfold'):
    super().__init__(name=name)
    self.config = config
  ...
  def __call__(
    ...
    def do_call(prev,
      ...
      if self.config.resample_msa_in_recycling:
        ...
        num_ensemble=1
        recycle_idx = recycle_idx%batch_size
...
class AlphaFoldBatch(hk.Module):
  """batched AlphaFold model. """

  def __init__(self, config, train_config, name='deepfold_batch'):
    super().__init__(name=name)
    self.config = config
    self.global_config = config.global_config
  ...
  def __call__(
    ...
    def body(loss, i):
      ...
      seq_length_weight = jnp.sqrt(jnp.sum(cur_batch['all_atom_mask'][0,:,0]))
      new_loss = new_loss * seq_length_weight / self.train_config.minibatch_size
      return loss+new_loss, jax.lax.stop_gradient(pred)
    
    if compute_loss:
      loss = 0.
      if self.train_config.minibatch_size == 1:
          loss, pred = body(loss, 0)
      else:
          loss, pred = hk.scan(body, loss, jnp.arange(0, self.train_config.minibatch_size))
      return pred, loss
    else:
      return model(
          batch,
...
class MaskedMsaHead(hk.Module):
  ...
  def loss(self, value, batch):
    ...
    jax_log('head/masked_msa', loss)
    #distillation loss by cy3
    if 'teacher_masked_msa_logits' in batch:
      dloss = _distillation_loss(value['logits'],
                        batch['teacher_masked_msa_logits'],
                        self.config.distillation)
      jax_log('head/masked_msa/distillation', dloss)
      loss+=dloss
    return {'loss': loss}
...
class PredictedLDDTHead(hk.Module):
  ...
  def loss(self, value, batch):
    ...
    jax_log('head/plddt', loss)
    #distillation loss by cy3
    if 'teacher_predicted_lddt_logits' in batch:
      dloss = _distillation_loss(value['predicted_lddt']['logits'],
                        batch['teacher_predicted_lddt_logits'],
                        self.config.distillation)
      jax_log('head/plddt/distillation', dloss)
      loss += dloss
...
class PredictedSCConfidenceHead(hk.Module):
  """Head to predict the per-residue per-angle (only for chi1) side chain confidence to be used as aother confidence measure.
  """

  def __init__(self, config, global_config, name='predicted_sc_confidence_head'):
    super().__init__(name=name)
    self.config = config
    self.global_config = global_config
  ...
  def __call__(self, representations, batch, is_training):
    ...
    Arguments:
      representations: Dictionary of representations, must contain:
        * 'angle_logits': Angle representation from the structure module,
             shape [N_res, c_a].
      batch: Batch, unused.
      is_training: Whether the module is in training mode.
    ...
    Returns:
      Dictionary containing :
        * 'logits': logits of shape [N_res, 7, N_bins] with
            (unnormalized) log probabilies of binned predicted side chain confidence.
    """
    act = representations['angle_logits']
    ...
    #logits = common_modules.Linear(
    #    self.config.num_bins * 7,
    #    initializer=utils.final_init(self.global_config),
    #    name='logits')(act)
    
    #logits = jnp.reshape(logits, [num_res, 7, self.config.num_bins])
    #only for chi1
    logits = common_modules.Linear(
        self.config.num_bins,
        initializer=utils.final_init(self.global_config),
    ...
    #logits = jnp.reshape(logits, [num_res, self.config.num_bins])
    # Shape (batch_size, num_res, 7, num_bins)
    return dict(logits=logits)
  ...
  def loss(self, value, batch):
    ...
    # Shape (num_res, 7)
    pred_angles = jnp.reshape(value['structure_module']['sidechains']['angles_sin_cos'], [-1, num_res, 7, 2])
    pred_angles = jnp.arctan2(pred_angles[-1,:,:,0], pred_angles[-1,:,:,1]) * 180 / jnp.pi
    target_tor_angles = pred_angles
    # Shape ()
    aatype = batch['aatype'][None]
    # Shape ()
    final_atom_positions = value['structure_module']['final_atom_positions'][None]
    # Shape ()
    final_atom_mask = value['structure_module']['final_atom_mask'][None]
    
    torsion_infos = all_atom.atom37_to_torsion_angles(aatype=aatype, all_atom_pos=final_atom_positions, all_atom_mask=final_atom_mask)
    # Remove these two line (below) after comparing torsion_angles_rmv with pred_angles
    #torsion_angles_rmv = torsion_infos['torsion_angles_sin_cos']
    #torsion_angles_rmv = jnp.arctan2(torsion_angles_rmv[-1,:,:,0], torsion_angles_rmv[-1,:,:,1]) * 180 / jnp.pi
    # Shape (num_res, 7)
    torsion_mask = torsion_infos['torsion_angles_mask']
    #logging.info(f'batch has {batch.keys()}')
    ret = all_atom.atom37_to_torsion_angles(aatype=aatype,
                                            all_atom_pos=batch['all_atom_positions'][None],
                                            all_atom_mask=batch['all_atom_mask'][None])
    native_tor_masks = ret['torsion_angles_mask']
    native_tor_angles = ret['torsion_angles_sin_cos']
    native_tor_angles = jnp.arctan2(native_tor_angles[-1,:,:,0], native_tor_angles[-1,:,:,1]) * 180 / jnp.pi
    native_tor_alt_angles = ret['alt_torsion_angles_sin_cos']
    
    ref_diff = 12.0
    diff = jnp.abs(native_tor_angles - target_tor_angles)
    diff = jnp.where(diff > 360 - diff, 360 - diff, diff)
    torsion_confidence = 1 / (1 + jnp.power((diff / ref_diff), 2))
    torsion_confidence = native_tor_masks * torsion_confidence
    
    curr_res_count = jnp.sum(native_tor_masks[:,:,3])  # only for chi1

    num_bins = self.config.num_bins
    bin_index = jnp.floor(torsion_confidence * num_bins).astype(jnp.int32)

    # protect against out of range for side chain confidence score == 1
    bin_index = jnp.minimum(bin_index, num_bins - 1)
    torsion_confidence_chi1_one_hot = jax.nn.one_hot(bin_index, num_classes=num_bins)

    # Shape (num_res, 7, num_channel)
    logits = value['predicted_sc_confidence']['logits']
    #errors = softmax_cross_entropy(labels=torsion_confidence_chi1_one_hot[:,:,3,:], logits=logits[:,3,:])
    errors = softmax_cross_entropy(labels=torsion_confidence_chi1_one_hot[:,:,3,:], logits=logits)

    # Shape (num_res,) ??????????
    loss = jnp.sum(errors * native_tor_masks[:,:,3]) / (curr_res_count + 1e-8)

    #distillation loss by cy3
    if 'teacher_predicted_sc_confidence_logits' in batch:
      loss += _distillation_loss(value['predicted_sc_confidence']['logits'],
                        batch['teacher_predicted_sc_confidence_logits'],
                        self.config.distillation)

    jax_log('head/sc_confidence', loss)
...
class ExperimentallyResolvedHead(hk.Module):
  ...
  def loss(self, value, batch):
    ...
    jax_log('head/experimentally_resolved', loss)
    #distillation loss by cy3
    if 'teacher_exp_resolved_logits' in batch:
      dloss = _distillation_loss(value['logits'],
                      batch['teacher_exp_resolved_logits'],
                      self.config.distillation)
      jax_log('head/experimentally_resolved/distillation', dloss)
      loss += dloss
...
# cy3
class DistillationHead(hk.Module):
  """Head to distllation learning for represenations. """

  def __init__(self, config, global_config, name='distillation_head'):
    super().__init__(name=name)
    self.config = config
    self.global_config = global_config
  ...
  def loss(self, value, batch):
    #distillation loss
    loss = 0.
    for repr_name, representation in value['representations'].items():
      if f'teacher_{repr_name}' in batch and self.config.get(f'{repr_name}.alpha', 0.0):
        if repr_name in ['ipa', 'backbone']:
          dloss = _distillation_mse_loss(representation[-1],
          #dloss = _distillation_loss(representation[-1],
                                      batch[f'teacher_{repr_name}'].squeeze()[-1],
                                      self.config[repr_name])
        else:
          dloss = _distillation_mse_loss(representation,
          #dloss = _distillation_loss(representation,
                                      batch[f'teacher_{repr_name}'],
                                      self.config[repr_name])
        #jax_print(repr_name)
        jax_log(f'head/distillation/{repr_name}', dloss)
        loss += dloss
    jax_log('head/distillation', loss)
    return {'loss': loss}
...
class DistogramHead(hk.Module):
  ...
  def loss(self, value, batch):
    loss_dict = _distogram_log_loss(value['logits'],
                                    value['bin_edges'],
                                    batch, self.config.num_bins)
    jax_log('head/distogram', loss_dict['loss'])
    #distillation loss by cy3
    if 'teacher_distogram_logits' in batch:
      dloss = _distillation_loss(value['logits'],
                                batch['teacher_distogram_logits'],
                                self.config.distillation)
      jax_log('head/distogram/distillation', dloss)
      loss_dict['loss']+=dloss
    return loss_dict

# cy3
def _distillation_loss(student, teacher:jnp.ndarray, params):
  """distillation loss."""
  alpha = params.alpha
  T = params.T
  teacher = teacher.squeeze()
  if student.shape != teacher.shape:
      return 0.
  logits = jax.nn.log_softmax(student/T).astype(jnp.float32)
  labels = jax.nn.log_softmax(teacher/T).astype(jnp.float32)
  loss = jnp.mean(jnp.exp(labels) * (labels - logits))
  return alpha * T * T * jnp.asarray(loss)

# cy3
def _distillation_mse_loss(student, teacher, params):
  """distillation mse loss."""
  teacher = teacher.squeeze()
  if student.shape != teacher.shape:
      return 0.
  alpha = params.alpha
  loss = jnp.mean((student - teacher)**2)
  return alpha * jnp.asarray(loss)
