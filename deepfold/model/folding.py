...
class InvariantPointAttention(hk.Module):
  ...
  def __call__(self, inputs_1d, inputs_2d, mask, affine):
    ...
    assert inputs_1d.dtype == inputs_2d.dtype
    ...
    attn_qk_point = -0.5 * jnp.sum(
        point_weights[:, None, None, :] * dist2, axis=-1).astype(dtype)

    v = jnp.swapaxes(v_scalar, -2, -3).astype(dtype)
    q = jnp.swapaxes(scalar_weights * q_scalar, -2, -3).astype(dtype)
    k = jnp.swapaxes(k_scalar, -2, -3).astype(dtype)
    ...
    attention_2d = (attention_2d_weights.astype(attention_2d.dtype) * attention_2d)
    attn_logits += attention_2d

    mask_2d = mask * jnp.swapaxes(mask, -1, -2)
    attn_logits -= 3e4 * (1. - mask_2d)

    # [num_head, num_query_residues, num_target_residues]
    attn = utils.softmax(attn_logits)
    ...
    result_point_global = [jnp.sum(
        attn[:, :, :, None] * vx[:, None, :, :],
        axis=-2).astype(dtype) for vx in v_point]
...
class FoldIteration(hk.Module):
  ...
  def __call__(self,
    ...
    if update_affine:
      ...
      #backbone activation
      backbone = affine_update

      affine = affine.pre_compose(affine_update)

    sc, angle_logits = MultiRigidSidechain(c.sidechain, self.global_config)(
        affine.scale_translation(c.position_scale), [act, initial_act], aatype)

    # secondary structure prediction
    act_ss = layer_norm.LayerNorm(
        axis=[-1],
        create_scale=True,
        create_offset=True,
        name='secondary_layer_norm')(
            act)
    act_ss = common_modules.Linear(
        c.num_channel,
        initializer='relu',
        name='secondary_act_0')(
            act_ss)
    act_ss = jax.nn.relu(act_ss)

    act_ss = common_modules.Linear(
        c.num_channel,
        initializer='relu',
        name='secondary_act_1')(
            act_ss)
    act_ss = jax.nn.relu(act_ss)
    ss = common_modules.Linear(
        c.secondary.num_output,
        initializer=final_init,
        name='secondary_logits')(
            act_ss)
    ...
    return new_activations, outputs, angle_logits
...
def generate_affines(representations, batch, config, global_config,
  ...
  for sub_key in safe_keys:
    activations, output, angle_logits = fold_iteration(
        activations,
  ...
  output['angle_logits'] = angle_logits
...
class StructureModule(hk.Module):
  ...
  def __call__(self, representations, batch, is_training,
    ...
    if self.compute_loss:
      ...
      no_loss_features = ['final_atom_positions', 'final_atom_mask',
                          'representations', 'sidechains']      # sidechains by newton
  ...
  def loss(self, value, batch):
    ...
    ss_loss = secondary_structure_loss(ret, value, batch, self.config)
    ss_loss = self.config.secondary.weight_frac*ss_loss
    jax_log('structure/seconary_structure', ss_loss)
    ...
    jax_log('structure/backbone', ret['loss'])
    jax_log('structure/sidechain', sc_loss['loss'])
    jax_log('structure/fape', sc_loss['fape'])
    ...
    #jsg modifying
    torsion_infos = all_atom.atom37_to_torsion_angles(aatype = batch['aatype'][None], all_atom_pos = batch['all_atom_positions'][None], all_atom_mask = batch['all_atom_mask'][None])
    batch['bb_angles_sin_cos'] = torsion_infos['torsion_angles_sin_cos'][0,:,0:3,:]
    batch['bb_mask'] = torsion_infos['torsion_angles_mask'][0,:,0:3]
    
    torsion_angles = torsion_infos['torsion_angles_sin_cos']
    batch['all_torsion_masks']  = torsion_infos['torsion_angles_mask'][0]
    batch['all_torsion_angles'] = jnp.arctan2(torsion_angles[0,:,:,0], torsion_angles[0,:,:,1]) * 180 / jnp.pi
    
    if self.config.chi_weight:
      supervised_chi_loss(ret, batch, value, self.config)
      jax_log('structure/chi_loss', ret['chi_loss'])
    
    if self.config.bb_weight:
      supervised_bb_loss(ret, batch, value, self.config)
      jax_log('structure/bb_loss', ret['bb_loss'])
    
    if self.config.pair_weight:
      supervised_pair_loss(ret, batch, value, self.config)
      jax_log('structure/pair_loss', ret['pair_loss'])
    
    lit_check = self.config.phipsi_lit_weight + self.config.phichi1_lit_weight + self.config.psichi1_lit_weight + self.config.chi1chi2_lit_weight

    if lit_check:
      get_lit_violation(ret, batch, value, self.config)
      jax_log('structure/phipsi_lit', ret['phipsi_lit'])
      jax_log('structure/phichi1_lit', ret['phichi1_lit'])
      jax_log('structure/psichi1_lit', ret['psichi1_lit'])

    if self.config.structural_violation_loss_weight:
      if 'violations' not in value:
        value['violations'] = find_structural_violations(
            batch, value['final_atom14_positions'], self.config)    
    #jax_print([ret["bb_loss"], ret["chi_loss"], ret["phipsi_lit"], ret["phichi1_lit"], ret["psichi1_lit"]])

    return ret

def get_lit_violation(ret, batch, value, config):   
  target_tor_masks  = batch['all_torsion_masks']
  target_tor_angles = jnp.arctan2(value['sidechains']['angles_sin_cos'][-1,:,:,0], value['sidechains']['angles_sin_cos'][-1,:,:,1]) * 180 / jnp.pi
  
  bins = jnp.linspace(-180,180,37)
  nans = jnp.zeros(target_tor_masks.shape[0])
  nans.at[:].set(jnp.nan)

  loss = 0

  ramachan = batch['PHI_PSI']
  angle_1, angle_2 = 1, 2
  angles_1  = jnp.where(jnp.logical_and(target_tor_masks[:,angle_1],target_tor_masks[:,angle_2]), target_tor_angles[:,angle_1], nans)
  angles_2  = jnp.where(jnp.logical_and(target_tor_masks[:,angle_1],target_tor_masks[:,angle_2]), target_tor_angles[:,angle_2], nans)
  target = jnp.histogram2d(angles_1, angles_2, [bins,bins])[0]
  target /= jnp.sum(target)
  loss = - jnp.nansum(ramachan * jnp.log(target+(1e-10)/(ramachan+1e-10)))
  ret['loss'] += config.phipsi_lit_weight * loss
  ret['phipsi_lit'] = loss
  
  ramachan = batch['PHI_CHI1']
  angle_1, angle_2 = 1, 3
  angles_1  = jnp.where(jnp.logical_and(target_tor_masks[:,angle_1],target_tor_masks[:,angle_2]), target_tor_angles[:,angle_1], nans)
  angles_2  = jnp.where(jnp.logical_and(target_tor_masks[:,angle_1],target_tor_masks[:,angle_2]), target_tor_angles[:,angle_2], nans)
  target = jnp.histogram2d(angles_1, angles_2, [bins,bins])[0]
  target /= jnp.sum(target)
  loss = - jnp.nansum(ramachan * jnp.log(target+(1e-10)/(ramachan+1e-10)))
  ret['loss'] += config.phichi1_lit_weight * loss
  ret['phichi1_lit'] = loss
  
  ramachan = batch['PSI_CHI1']
  angle_1, angle_2 = 2, 3
  angles_1  = jnp.where(jnp.logical_and(target_tor_masks[:,angle_1],target_tor_masks[:,angle_2]), target_tor_angles[:,angle_1], nans)
  angles_2  = jnp.where(jnp.logical_and(target_tor_masks[:,angle_1],target_tor_masks[:,angle_2]), target_tor_angles[:,angle_2], nans)
  target = jnp.histogram2d(angles_1, angles_2, [bins,bins])[0]
  target /= jnp.sum(target)
  loss = - jnp.nansum(ramachan * jnp.log(target+(1e-10)/(ramachan+1e-10)))
  ret['loss'] += config.psichi1_lit_weight * loss  
  ret['psichi1_lit'] = loss
    
  ramachan = batch['CHI1_CHI2']
  angle_1, angle_2 = 3, 4
  angles_1  = jnp.where(jnp.logical_and(target_tor_masks[:,angle_1],target_tor_masks[:,angle_2]), target_tor_angles[:,angle_1], nans)
  angles_2  = jnp.where(jnp.logical_and(target_tor_masks[:,angle_1],target_tor_masks[:,angle_2]), target_tor_angles[:,angle_2], nans)
  target = jnp.histogram2d(angles_1, angles_2, [bins,bins])[0]
  target /= jnp.sum(target)
  loss = - jnp.nansum(ramachan * jnp.log(target+(1e-10)/(ramachan+1e-10)))
  ret['loss'] += config.chi1chi2_lit_weight * loss 
  ret['chi1chi2_lit'] = loss
...
#Secondary_Structure_loss (choo)
def secondary_structure_loss(ret, value, batch, config):
  errors = softmax_cross_entropy(labels=batch['ss_type'], logits = value['secondary'])
  loss = jnp.sum(errors*batch['ss_mask'] / jnp.sum(1e-8+batch['ss_mask']))
  return loss
...
# jhw - using distogram for weighting FAPE
def get_argmaxed_distogram(logits):
    if logits is None:
        return None
    
    bin_all_edges = np.linspace(2, 22, 65)
    bin_mid_values = []
    for i in range(1, 65):
        bin_mid_values.append((bin_all_edges[i - 1] + bin_all_edges[i]) / 2)
    bin_mid_values = np.asarray(bin_mid_values)

    softmaxed_logits = utils.softmax(logits)
    argmaxed_idx = jnp.argmax(softmaxed_logits, axis=-1)

    n_res = argmaxed_idx.shape[0]

    bin_mat = np.zeros((n_res, n_res, 64))
    bin_mat[
        :,
        :,
    ] = bin_mid_values  # this makes (n_res, n_res, 64) pairwsied bin matrix

    i, j = jnp.ogrid[:n_res, :n_res]
    disto_ij = jnp.asarray(bin_mat)[i, j, argmaxed_idx]
...
def sidechain_loss(batch, value, config):
  ...
  # jhw - using distogram for weighting FAPE
  predicted_distogram = get_argmaxed_distogram(batch["distogram"]['logits'])
  ...
  fape = all_atom.frame_aligned_point_error(
      ...
      # jhw - using distogram for weighting FAPE
      predicted_distogram=predicted_distogram,
      weight_params=[
          config.sidechain.weight_param_alpha,
          config.sidechain.weight_param_h_offset,
          config.sidechain.weight_param_v_offset
      ],
...
def supervised_bb_loss(ret, batch, value, config):
  ...
  bb_mask = batch['bb_mask'].astype(jnp.float32)
  ...
  sin_cos_true_bb = batch["bb_angles_sin_cos"][None]

  sq_bb_error = jnp.nansum(
      squared_difference(sin_cos_true_bb, pred_angles), -1) + 1e-4
  
  if   config.loss_type == 'root': sq_bb_error   = sq_bb_error **(1/4)
  elif config.loss_type == 'normal': sq_bb_error = sq_bb_error **(1/2)
  elif config.loss_type == 'square': sq_bb_error = sq_bb_error

  sq_bb_loss = utils.mask_mean(mask=bb_mask[None], value=sq_bb_error)
  
  ret['bb_loss'] = sq_bb_loss
  ret['loss'] += config.bb_weight * sq_bb_loss
...
def supervised_chi_loss(ret, batch, value, config):
  ...
  sq_chi_error = jnp.nansum(
      squared_difference(sin_cos_true_chi, pred_angles), -1)
  sq_chi_error_shifted = jnp.nansum(
      squared_difference(sin_cos_true_chi_shifted, pred_angles), -1)
  sq_chi_error = jnp.minimum(sq_chi_error, sq_chi_error_shifted) + 1e-4
  
  if   config.loss_type == 'root': 
    sq_chi_error   = sq_chi_error **(1/4)
    l_max = jnp.sqrt(2)
  elif config.loss_type == 'normal': 
    sq_chi_error = sq_chi_error **(1/2)
    l_max = 2
  elif config.loss_type == 'square': 
    sq_chi_error = sq_chi_error
    l_max = 4

  chi1_error = sq_chi_error[...,0] * config.chi1_weight
  chi2_error = sq_chi_error[...,1] * config.chi2_weight
  chi3_error = sq_chi_error[...,2] * config.chi3_weight
  chi4_error = sq_chi_error[...,3] * config.chi4_weight
  
  if   config.chi_dependent : 
  ...
  sq_chi_error = jnp.concatenate([chi1_error[...,None],chi2_error[...,None], chi3_error[...,None], chi4_error[...,None]], axis = -1)
...
def supervised_pair_loss(ret, batch, value, config):
    """Computes loss for direct chi angle supervision.
    ...
    bb_angles  = pred_angles[:,:,:3,:]
    chi_angles = pred_angles[:,:,3:,:]
    ...
    sq_chi_error = jnp.sum(squared_difference(sin_cos_true_chi, chi_angles), -1)
    sq_chi_error_shifted = jnp.sum(
        squared_difference(sin_cos_true_chi_shifted, chi_angles), -1
    )
    sq_chi_error = jnp.minimum(sq_chi_error, sq_chi_error_shifted)
    
    sq_bb_error = jnp.sum(
      squared_difference(sin_cos_true_bb, bb_angles), -1) 
    
    phi_psi   = jnp.maximum(sq_bb_error[:,:,1], config.pair_epsilon) + jnp.maximum(sq_bb_error[:,:,1], config.pair_epsilon)
    phi_chi1  = jnp.maximum(sq_bb_error[:,:,1], config.pair_epsilon) + jnp.maximum(sq_chi_error[:,:,0], config.pair_epsilon)
    psi_chi1  = jnp.maximum(sq_bb_error[:,:,2], config.pair_epsilon) + jnp.maximum(sq_bb_error[:,:,0], config.pair_epsilon)
    chi1_chi2 = jnp.maximum(sq_chi_error[:,:,0], config.pair_epsilon) +jnp.maximum(sq_bb_error[:,:,1], config.pair_epsilon)
    
    sq_phi_psi_loss   = utils.mask_mean(mask=jnp.logical_and(bb_mask[None][:,:,1], bb_mask[None][:,:,2]), value=phi_psi)
    sq_phi_chi1_loss  = utils.mask_mean(mask=jnp.logical_and(bb_mask[None][:,:,1], chi_mask[None][:,:,0]), value=phi_chi1)
    sq_psi_chi1_loss  = utils.mask_mean(mask=jnp.logical_and(bb_mask[None][:,:,2], chi_mask[None][:,:,0]), value=phi_psi)
    sq_chi1_chi2_loss = utils.mask_mean(mask=jnp.logical_and(chi_mask[None][:,:,0], chi_mask[None][:,:,1]), value=chi1_chi2)

    ret["pair_loss"] = sq_phi_psi_loss + sq_phi_chi1_loss + sq_psi_chi1_loss + sq_chi1_chi2_loss
    ret["loss"] += config.pair_weight * ret['pair_loss']
