...
# jhw - weighting function for FAPE 
def weight_dist(d_ij, alpha, h_offset, v_offset):
    weights = 1 / (1 + jnp.exp(-alpha * (-d_ij + h_offset))) + v_offset
    return weights
...
def frame_aligned_point_error(
    ...
    predicted_distogram: Optional[jnp.ndarray] = None,
    weight_params: Optional[list] = None,
  ...
  if l1_clamp_distance:
    ...
    # jhw - weighting FAPE if a distogram exists
    if (predicted_distogram is not None) and not (None in weight_params):
      alpha = weight_params[0] # sharpness of h_offset
      h_offset = weight_params[1] # a value where the sigmoid value is equal to 0.5
      v_offset = weight_params[2] # weighting scale
      weights = weight_dist(predicted_distogram, alpha, h_offset, v_offset)
      
      weights = jnp.kron(weights, jnp.ones((8, 14), dtype=weights.dtype))
      
      if error_dist.shape == weights.shape:
          # weighting only unclamped area
          error_dist = jnp.where(error_dist < 10.0, 
                                 jnp.multiply(error_dist, weights), 
                                 error_dist)
