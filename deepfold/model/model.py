...
from deepfold.common import confidence
from deepfold.model import features
from deepfold.model import modules
from deepfold.train.data_system import cast_to_precision
from deepfold.train.mixed_precision import normalize_precision, cast_floating_to
...
def get_confidence_metrics(
  ...
  if 'predicted_sc_confidence' in prediction_result:
    confidence_metrics['p_sc_conf'] = confidence.compute_sc_confidence(
        prediction_result['predicted_sc_confidence']['logits'])
...
class RunModel:
  ...
  def __init__(self,
    ...
    self.config = config
    if num_recycle:
      self.config.model.num_recycle = num_recycle
    self.params = params
    ...
    self.batch_config = ConfigDict({'minibatch_size': 1})

    def _forward_fn(batch):
      model = modules.AlphaFoldBatch(self.config.model, self.batch_config)
      ret = model(
          batch,
          is_training=False,
          compute_loss=False,
          return_representations=self.return_representations,
          ensemble_representations=self.ensemble_representations)
