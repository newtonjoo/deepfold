...
def compute_sc_confidence(logits: np.ndarray) -> np.ndarray:
  """Computes per-residue p_sc_confidence from logits.

  Args:
    logits: [num_res, num_bins] output from the PredictedSCConfidenceHead.

  Returns:
    p_sc_confidence: [num_res] per-residue p_sc_confidence for chi1.
  """
  num_bins = logits.shape[-1]
  bin_width = 1.0 / num_bins
  bin_centers = np.arange(start=0.5 * bin_width, stop=1.0, step=bin_width)
  #probs = scipy.special.softmax(logits[:,3], axis=-1)  # only for chi1
  probs = scipy.special.softmax(logits, axis=-1)  # only for chi1
  predicted_sc_conf_chi1 = np.sum(probs * bin_centers[None, :], axis=-1)
  return predicted_sc_conf_chi1 * 100
