...
"""Run DeepFold with preprocessed protein features (features.pkl)."""
...
from deepfold.inference.inference_pipeline import predict_from_pkl
from deepfold.model.config import model_config as get_model_config
from deepfold.model.model import RunModel
from deepfold.train.mixed_precision import normalize_precision
...
flags.DEFINE_list(
    ...
    '`deepfold/model/config.py`')
flags.DEFINE_string(
    'repr_path', None, 'Path to representation pkl file')
flags.DEFINE_list(
...
def main(argv):
  ...
  for model_name, model_path in zip(FLAGS.model_names, FLAGS.model_paths):
    ...
    if FLAGS.repr_path and FLAGS.repr_path != "None":
      model_runner = RunModel(
         config=model_config,
         params=model_params,
         precision=precision,
      return_representations=False, ensemble_representations=False)
    else:
      model_runner = RunModel(
         config=model_config,
         params=model_params,
  ...
  for id, feature_path in protein_dict.items():
    ...
    representation = None
    if FLAGS.repr_path and FLAGS.repr_path != "None":
      with open(FLAGS.repr_path, 'rb') as fp:
        representation = pickle.load(fp)
    try:
      predict_from_pkl(
          features=features,
          name=id,
